...
export function DeckImporter() {
  ...
  const cardNames = deck ? Array.from(new Set([
    // v3 API structure - extract names from card data
    ...(deck.boards?.mainboard?.cards ? 
      Object.values(deck.boards.mainboard.cards).map(card => card.card?.name || '').filter(Boolean) : []),
    ...(deck.boards?.sideboard?.cards ? 
      Object.values(deck.boards.sideboard.cards).map(card => card.card?.name || '').filter(Boolean) : []),
        ...
        <div className="space-y-4">
          <div className="bg-secondary rounded-lg p-4">
            ...
            <p className="text-muted-foreground">
              ...
              {' '}{Object.values(getMainboardCards(deck)).reduce((sum, card) => sum + card.quantity, 0)} total cards
            </p>
          </div>
