...
import { DualFacedCardPreview, DualFacedIndicator } from "@/components/ui/dual-faced-card";
...
interface CategoryEvaluationCardProps {
  ...
  cardsData?: CardAggregate[]; // Optional full card data for dual-faced support
  description: string;
...
export function CategoryEvaluationCard({ 
  ...
  // Helper to find full card data by name
  const getFullCardData = (cardName: string): CardAggregate | undefined => {
    return cardsData?.find(c => c.name === cardName);
  };
          ...
          {isExpanded && (
            <div className="mt-2 space-y-1">
              {cards.map((card, idx) => {
                ...
                const isDualFaced = fullCard && fullCard.cardFaces && fullCard.cardFaces.length > 1;
                ...
                const nameElement = (
                  <div className="flex items-center gap-2">
                    ...
                    {isDualFaced && <DualFacedIndicator card={fullCard} className="scale-75" />}
                  </div>
                );
                ...
                return (
                  <div key={idx} className="flex items-center gap-2 p-1 hover:bg-accent/50 rounded">
                    {card.imageUrl && (
                      isDualFaced && fullCard ? (
                        <DualFacedCardPreview card={fullCard}>
                          {imageElement}
